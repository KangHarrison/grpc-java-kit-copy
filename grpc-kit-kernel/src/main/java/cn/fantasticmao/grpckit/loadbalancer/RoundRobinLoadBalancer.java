...
import cn.fantasticmao.grpckit.loadbalancer.picker.RoundRobinPicker;
 ...
 * @since 2022/4/24
 */
public class RoundRobinLoadBalancer extends ServiceLoadBalancer {
        ...
        for (Map.Entry<List<SocketAddress>, EquivalentAddressGroup> latestEntry : latestAddressGroupMap.entrySet()) {
            ...
            final Subchannel subChannel = helper.createSubchannel(CreateSubchannelArgs.newBuilder()
                ...
                .setAttributes(Attributes.newBuilder()
                    ...
                    .build())
                .build());
            subChannel.start(new RoundRobinLoadBalancer.StateListener(subChannel));
    ...
    private void updateBalancingState() {
        ...
        if (!readySubChannelList.isEmpty()) {
            updateBalancingState(READY, new RoundRobinPicker(readySubChannelList));
        } else {
    ...
    final class StateListener implements LoadBalancer.SubchannelStateListener {
        ...
        public void onSubchannelState(ConnectivityStateInfo newState) {
            ...
            if (RoundRobinLoadBalancer.this.subChannelMap.get(addressList) != subChannel) {
                LOGGER.warn("The two subChannels are not the same when state changes.");
                return;
            }
            if (newState.getState() == TRANSIENT_FAILURE || newState.getState() == IDLE) {
                RoundRobinLoadBalancer.this.helper.refreshNameResolution();
            }
            ...
            RoundRobinLoadBalancer.this.updateBalancingState();
        }
    }
}
